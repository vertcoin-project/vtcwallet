...
// InputSourceError.
type InputSource func(target vtcutil.Amount) (total vtcutil.Amount, inputs []*wire.TxIn,
	inputValues []vtcutil.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []vtcutil.Amount
	TotalInput      vtcutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb vtcutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []vtcutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := vtcutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
