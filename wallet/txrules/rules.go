...
const DefaultRelayFeePerKb vtcutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb vtcutil.Amount) vtcutil.Amount {
	...
	relayFee := vtcutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount vtcutil.Amount, scriptSize int, relayFeePerKb vtcutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb vtcutil.Amount) bool {
	...
	return IsDustAmount(vtcutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb vtcutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > vtcutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb vtcutil.Amount, txSerializeSize int) vtcutil.Amount {
	fee := relayFeePerKb * vtcutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > vtcutil.MaxSatoshi {
		fee = vtcutil.MaxSatoshi
	}
