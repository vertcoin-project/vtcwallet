...
func (w *Wallet) MakeMultiSigScript(addrs []vtcutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*vtcutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*vtcutil.AddressScriptHash, error) {
	var p2shAddr *vtcutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = vtcutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*vtcutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
