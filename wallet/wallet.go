...
func (w *Wallet) RelayFee() vtcutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee vtcutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
// rescan request.
func (w *Wallet) activeData(dbtx walletdb.ReadTx) ([]vtcutil.Address, []wtxmgr.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)

	var addrs []vtcutil.Address
	err := w.Manager.ForEachActiveAddress(addrmgrNs, func(addr vtcutil.Address) error {
...
// account.
func (w *Wallet) AccountAddresses(account uint32) (addrs []vtcutil.Address, err error) {
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// include a UTXO.
func (w *Wallet) CalculateBalance(confirms int32) (vtcutil.Amount, error) {
	var balance vtcutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
...
type Balances struct {
	Total          vtcutil.Amount
	Spendable      vtcutil.Amount
	ImmatureReward vtcutil.Amount
}
...
// one transaction spending to it in the blockchain or vtcd mempool), the next
// chained address is returned.
func (w *Wallet) CurrentAddress(account uint32) (vtcutil.Address, error) {
	var addr vtcutil.Address
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) PubKeyForAddress(a vtcutil.Address) (*btcec.PublicKey, error) {
...
// address.
func (w *Wallet) PrivKeyForAddress(a vtcutil.Address) (*btcec.PrivateKey, error) {
...
func (w *Wallet) HaveAddress(a vtcutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a vtcutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a vtcutil.Address) (waddrmgr.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal vtcutil.Amount
		for _, deb := range details.Debits {
			debitTotal += deb.Amount
		}
		var outputTotal vtcutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += vtcutil.Amount(output.Value)
		}
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := vtcutil.Amount(output.Value).ToBTC()
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*vtcutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*vtcutil.Amount)
...
func (w *Wallet) DumpPrivKeys() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// privkeys.
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr vtcutil.Address) error {
...
func (w *Wallet) DumpWIFPrivateKey(addr vtcutil.Address) (string, error) {
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *vtcutil.WIF, bs *waddrmgr.BlockStamp,
	...
	if rescan {
		...
		err := w.chainClient.NotifyReceived([]vtcutil.Address{addr})
		if err != nil {
...
func (w *Wallet) NewAddress(account uint32, addrType waddrmgr.AddressType) (addr vtcutil.Address, err error) {
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	addrType waddrmgr.AddressType) (vtcutil.Address, error) {
	...
	// Request updates from vtcd for new transactions sent to this address.
	utilAddrs := make([]vtcutil.Address, len(addrs))
...
func (w *Wallet) NewChangeAddress(account uint32, addrType waddrmgr.AddressType) (addr vtcutil.Address, err error) {
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		addrmgrNs := tx.ReadWriteBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32, addrType waddrmgr.AddressType) (vtcutil.Address, error) {
...
// address.
func (w *Wallet) TotalReceivedForAddr(addr vtcutil.Address, minConf int32) (vtcutil.Amount, error) {
	var amount vtcutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*vtcutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr vtcutil.Address) (*btcec.PrivateKey, bool, error) {
			...
			getScript := txscript.ScriptClosure(func(addr vtcutil.Address) ([]byte, error) {
