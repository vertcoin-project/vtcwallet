...
func fetchMinedBalance(ns walletdb.ReadBucket) (vtcutil.Amount, error) {
	...
	return vtcutil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt vtcutil.Amount) error {
	v := make([]byte, 8)
...
func fetchRawCreditAmount(v []byte) (vtcutil.Amount, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (vtcutil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketCredits, 9, len(v))
		return 0, false, storeError(ErrData, str, nil)
	}
	return vtcutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (vtcutil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketCredits, 9, len(v))
		return 0, false, storeError(ErrData, str, nil)
	}
	return vtcutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spendCredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (vtcutil.Amount, error) {
	...
	return vtcutil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (vtcutil.Amount, error) {
	...
	return vtcutil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Amount = vtcutil.Amount(byteOrder.Uint64(it.cv))
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount vtcutil.Amount, block *Block, credKey []byte) error {
...
func valueUnminedCredit(amount vtcutil.Amount, change bool) []byte {
	v := make([]byte, 9)
	byteOrder.PutUint64(v, uint64(amount))
	if change {
		v[8] = 1 << 1
...
func fetchRawUnminedCreditAmount(v []byte) (vtcutil.Amount, error) {
	if len(v) < 9 {
...
func fetchRawUnminedCreditAmountChange(v []byte) (vtcutil.Amount, bool, error) {
	if len(v) < 9 {
		str := "short unmined credit value"
		return 0, false, storeError(ErrData, str, nil)
	}
	amt := vtcutil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
