...
func getAddressKey(addr vtcutil.Address) addressKey {
...
type Store struct {
	...
	passphrase       []byte
	secret           []byte
	chainIdxMap      map[int64]vtcutil.Address
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	s := &Store{
		...
		chainIdxMap:      make(map[int64]vtcutil.Address),
...
func (s *Store) ReadFrom(r io.Reader) (n int64, err error) {
	...
	s.chainIdxMap = make(map[int64]vtcutil.Address)
...
func (s *Store) NextChainedAddress(bs *BlockStamp) (vtcutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ChangeAddress(bs *BlockStamp) (vtcutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) LastChainedAddress() vtcutil.Address {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) Address(a vtcutil.Address) (WalletAddress, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) SetSyncStatus(a vtcutil.Address, ss SyncStatus) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ImportPrivateKey(wif *vtcutil.WIF, bs *BlockStamp) (vtcutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	pkh := vtcutil.Hash160(wif.SerializePubKey())
...
func (s *Store) ImportScript(script []byte, bs *BlockStamp) (vtcutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	if _, ok := s.addrMap[addressKey(vtcutil.Hash160(script))]; ok {
		return nil, ErrDuplicate
	}
...
func (s *Store) ActiveAddresses() map[vtcutil.Address]WalletAddress {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	addrs := make(map[vtcutil.Address]WalletAddress)
	for i := int64(rootKeyChainIdx); i <= s.highestUsed; i++ {
		a := s.chainIdxMap[i]
...
//
// A slice is returned with the vtcutil.Address of each new address.
// The blockchain must be rescanned for these addresses.
func (s *Store) ExtendActiveAddresses(n int) ([]vtcutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func newBtcAddressWithoutPrivkey(s *Store, pubkey, iv []byte, bs *BlockStamp) (addr *btcAddress, err error) {
	...
	address, err := vtcutil.NewAddressPubKeyHash(vtcutil.Hash160(pubkey), s.netParams())
	if err != nil {
		return nil, err
	}
...
func (a *btcAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	addr, err := vtcutil.NewAddressPubKeyHash(pubKeyHash[:], a.store.netParams())
	if err != nil {
		return n, err
	}
	a.address = addr
...
func (a *btcAddress) Address() vtcutil.Address {
	return a.address
}
...
func (a *btcAddress) ExportPrivKey() (*vtcutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}
	...
	// elsewhere.
	wif, err := vtcutil.NewWIF((*btcec.PrivateKey)(pk), a.store.netParams(),
		a.Compressed())
	if err != nil {
		panic(err)
	}
...
func newScriptAddress(s *Store, script []byte, bs *BlockStamp) (addr *scriptAddress, err error) {
	...
	scriptHash := vtcutil.Hash160(script)

	address, err := vtcutil.NewAddressScriptHashFromHash(scriptHash, s.netParams())
	if err != nil {
		return nil, err
	}
...
// address returns a vtcutil.AddressScriptHash for a btcAddress.
func (sa *scriptAddress) Address() vtcutil.Address {
	return sa.address
}
...
func (sa *scriptAddress) Addresses() []vtcutil.Address {
	return sa.addresses
}
