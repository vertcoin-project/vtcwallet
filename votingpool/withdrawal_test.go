...
func TestStartWithdrawal(t *testing.T) {
	...
	checkWithdrawalOutputs(t, status, map[string]vtcutil.Amount{address1: 4e6, address2: 1e6})

	if status.Fees() != vtcutil.Amount(1e3) {
		t.Fatalf("Wrong amount for fees; got %v, want %v", status.Fees(), vtcutil.Amount(1e3))
	}
...
func checkWithdrawalOutputs(
	t *testing.T, wStatus *vp.WithdrawalStatus, amounts map[string]vtcutil.Amount) {
