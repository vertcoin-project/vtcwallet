...
func TestOutputSplittingNotEnoughInputs(t *testing.T) {
	...
	output1Amount := vtcutil.Amount(2)
	output2Amount := vtcutil.Amount(3)
...
func TestOutputSplittingOversizeTx(t *testing.T) {
	...
	requestAmount := vtcutil.Amount(5)
	bigInput := int64(3)
	smallInput := int64(2)
	...
	if tx1.outputs[0].amount != vtcutil.Amount(bigInput) {
	...
	if tx2.outputs[0].amount != vtcutil.Amount(smallInput) {
...
func TestFulfillRequestsNoSatisfiableOutputs(t *testing.T) {
	...
	request := TstNewOutputRequest(
		t, 1, "3Qt1EaKRD9g9FeL2DGkLLswhK1AKmmXFSe", vtcutil.Amount(3e6), pool.Manager().ChainParams())
	changeStart := TstNewChangeAddress(t, pool, seriesID, 0)
...
func TestFulfillRequestsNotEnoughCreditsForAllRequests(t *testing.T) {
	...
	out1 := TstNewOutputRequest(
		t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yycntMK6", vtcutil.Amount(3e6), net)
	out2 := TstNewOutputRequest(
		t, 2, "3PbExiaztsSYgh6zeMswC49hLUwhTQ86XG", vtcutil.Amount(2e6), net)
	out3 := TstNewOutputRequest(
		t, 3, "3Qt1EaKRD9g9FeL2DGkLLswhK1AKmmXFSe", vtcutil.Amount(5e6), net)
...
func TestRollbackLastOutputMultipleInputsRolledBack(t *testing.T) {
	...
	for i, amount := range []vtcutil.Amount{4, 3, 2} {
...
func TestRollBackLastOutputInsufficientOutputs(t *testing.T) {
	...
	output := &WithdrawalOutput{request: TstNewOutputRequest(
		t, 1, "34eVkREKgvvGASZW7hkgE2uNc1yycntMK6", vtcutil.Amount(3), &chaincfg.MainNetParams)}
...
func TestRollbackLastOutputWhenNewOutputAdded(t *testing.T) {
	...
	checkTxChangeAmount(t, firstTx, vtcutil.Amount(4))
	...
	checkTxChangeAmount(t, secondTx, vtcutil.Amount(3))
}
...
func TestWithdrawalTxAddChange(t *testing.T) {
	...
	tx.calculateFee = TstConstantFee(vtcutil.Amount(fee))
...
func TestWithdrawalTxInputTotal(t *testing.T) {
	...
	if tx.inputTotal() != vtcutil.Amount(5) {
		t.Fatalf("Wrong total output; got %v, want %v", tx.outputTotal(), vtcutil.Amount(5))
	}
}
...
func TestWithdrawalTxOutputTotal(t *testing.T) {
	...
	if tx.outputTotal() != vtcutil.Amount(4) {
		t.Fatalf("Wrong total output; got %v, want %v", tx.outputTotal(), vtcutil.Amount(4))
	}
}
...
func TestSignMultiSigUTXOPkScriptNotP2SH(t *testing.T) {
	...
	mgr := pool.Manager()
	tx := createWithdrawalTx(t, dbtx, pool, []int64{4e6}, []int64{})
	addr, _ := vtcutil.DecodeAddress("1MirQ9bwyQcGVJPwKUgapu5ouK2E2Ey4gX", mgr.ChainParams())
	pubKeyHashPkScript, _ := txscript.PayToAddrScript(addr.(*vtcutil.AddressPubKeyHash))
	msgtx := tx.toMsgTx()
...
func TestSignMultiSigUTXORedeemScriptNotFound(t *testing.T) {
	...
	// script.
	addr, _ := vtcutil.DecodeAddress("3Hb4xcebcKg4DiETJfwjh8sF4uDw9rqtVC", mgr.ChainParams())
	if _, err := mgr.Address(addrmgrNs, addr); err == nil {
		t.Fatalf("Address %s found in manager when it shouldn't", addr)
	}
	msgtx := tx.toMsgTx()

	pkScript, _ := txscript.PayToAddrScript(addr.(*vtcutil.AddressScriptHash))
...
func TestStoreTransactionsWithChangeOutput(t *testing.T) {
	...
	inputTotal := vtcutil.Amount(0)
	for _, debit := range txDetails.Debits {
		inputTotal += debit.Amount
	}
	if inputTotal != vtcutil.Amount(5e6) {
		t.Fatalf("Unexpected input amount; got %v, want %v", inputTotal, vtcutil.Amount(5e6))
	}
...
func createWithdrawalTxWithStoreCredits(t *testing.T, dbtx walletdb.ReadWriteTx, store *wtxmgr.Store, pool *Pool,
	...
	for i, amount := range outputAmounts {
		request := TstNewOutputRequest(
			t, uint32(i), "34eVkREKgvvGASZW7hkgE2uNc1yycntMK6", vtcutil.Amount(amount), net)
...
func checkMsgTxOutputs(t *testing.T, msgtx *wire.MsgTx, requests []OutputRequest) {
	...
	for i, request := range requests {
		...
		gotAmount := vtcutil.Amount(txOut.Value)
...
func checkTxChangeAmount(t *testing.T, tx *withdrawalTx, amount vtcutil.Amount) {
	if !tx.hasChange() {
...
func checkLastOutputWasSplit(t *testing.T, w *withdrawal, tx *withdrawalTx,
	origAmount, newAmount vtcutil.Amount) {
