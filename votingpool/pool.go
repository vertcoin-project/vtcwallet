...
func (p *Pool) DepositScriptAddress(seriesID uint32, branch Branch, index Index) (vtcutil.Address, error) {
	script, err := p.DepositScript(seriesID, branch, index)
	if err != nil {
		return nil, err
	}
...
func (p *Pool) addressFor(script []byte) (vtcutil.Address, error) {
	scriptHash := vtcutil.Hash160(script)
	return vtcutil.NewAddressScriptHashFromHash(scriptHash, p.manager.ChainParams())
}
...
func (p *Pool) DepositScript(seriesID uint32, branch Branch, index Index) ([]byte, error) {
	...
	pks := make([]*vtcutil.AddressPubKey, len(pubKeys))
	for i, key := range pubKeys {
		...
		pks[i], err = vtcutil.NewAddressPubKey(pubkey.SerializeCompressed(),
...
func (p *Pool) addUsedAddr(ns, addrmgrNs walletdb.ReadWriteBucket, seriesID uint32, branch Branch, index Index) error {
	...
	encryptedHash, err := p.manager.Encrypt(waddrmgr.CKTPublic, vtcutil.Hash160(script))
	if err != nil {
...
func (p *Pool) getUsedAddr(ns, addrmgrNs walletdb.ReadBucket, seriesID uint32, branch Branch, index Index) (
	...
	addr, err := vtcutil.NewAddressScriptHashFromHash(hash, mgr.ChainParams())
	if err != nil {
